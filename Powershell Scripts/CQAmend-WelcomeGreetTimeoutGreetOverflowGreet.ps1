...
Set-CsCallQueue -Identity $CallQueueID -OverflowThreshold $OverflowThreshold -TimeoutThreshold $TimeoutThreshold -RoutingMethod $RoutingMethodNS -PresenceBasedRouting $ConvertedPresRouting -UseDefaultMusicOnHold $True -DistributionLists $GroupID -AgentAlertTime $AgentAlertTime -AllowOptOut $ConvertedAllowOptOut -WelcomeTextToSpeechPrompt $WelcomeTextToSpeech -OverflowAction DisconnectWithBusy -OverflowDisconnectTextToSpeechPrompt $OverflowTextToSpeech -TimeoutAction Disconnect -TimeoutDisconnectTextToSpeechPrompt $TimeoutTextToSpeech -ConferenceMode $True -LanguageID "en-GB" -ErrorAction Stop
