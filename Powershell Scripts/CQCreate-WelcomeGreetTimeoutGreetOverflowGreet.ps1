...
$NewCallQueue = New-CsCallQueue -Name $CallQueueDisplayName -OverflowThreshold $OverflowThreshold -TimeoutThreshold $TimeoutThreshold -RoutingMethod $RoutingMethodNS -PresenceBasedRouting $ConvertedPresRouting -UseDefaultMusicOnHold $True -DistributionLists $GroupID -AgentAlertTime $AgentAlertTime -AllowOptOut $ConvertedAllowOptOut -WelcomeTextToSpeechPrompt $WelcomeTextToSpeech -TimeoutAction "Disconnect" -TimeoutDisconnectTextToSpeechPrompt $TimeoutTextToSpeech -OverflowAction "DisconnectWithBusy" -OverflowDisconnectTextToSpeechPrompt $OverflowTextToSpeech -ConferenceMode $True -LanguageID "en-GB" -ErrorAction Stop
